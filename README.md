...
- `/admin`
  이벤트·방명록·고객을 운영하는 관리자 화면. 이벤트 상세에서 지원하는 기능에 따라 이미지, 추억, 방명록, 노출 기간, 고객 연결 관리로 이어집니다.
- `/page-wizard`
  관리자용 결혼식 페이지 생성 화면
- `/birthday-wizard`
  관리자용 생일 초대장 생성 화면
...
관리자 화면은 아래 세 업무 영역을 관리합니다.

- 이벤트: 전체 이벤트 검색, 상태 확인, 편집과 상세 관련 관리 진입
- 방명록: 이벤트별 댓글 조회와 삭제
- 고객: 고객 계정, 이벤트 연결, 제작권 운영
...
`/admin`은 이벤트·방명록·고객 세 업무 영역으로 구성된 운영 화면입니다.

- 이벤트: 전체 이벤트를 검색하고 공개·연결 상태를 확인합니다. 상세에서 편집과 함께 이벤트가 지원하는 이미지, 추억, 방명록, 노출 기간, 고객 연결 관리로 이동합니다.
- 방명록: 이벤트별 방명록을 조회하고 삭제합니다.
- 고객: 고객 계정, 이벤트 연결, 제작권을 운영합니다.
