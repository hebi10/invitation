...
기본형의 정보 순서와 기존 기능을 공통으로 사용하고, 사진 배치·제목·장식·갤러리 구성에서 차이를 만듭니다. 공개 화면의 최대 폭 480px을 유지하며 모바일에서 세로로 읽기 편하게 구성합니다. 디자인 선택 화면에는 기본형, 모던형, 포토형, 클래식형, 내추럴형 순서로 표시합니다.

| 표시 이름 | 기존 키 | 시각적 방향 |
| --- | --- | --- |
| 모던형 | `classic-r` | 흰 바탕과 검정 고딕 글씨, 번호가 있는 섹션, 잡지처럼 정돈된 사진 배치 |
| 포토형 | `romantic` | 따뜻한 종이색, 큰 대표 사진, 여러 장의 사진이 이어지는 앨범 구성 |
| 클래식형 | `gyeol` | 크림색 바탕과 갈색 명조 제목, 낮은 채도의 금색 장식, 격식을 갖춘 사진 프레임 |
| 내추럴형 | `emotional` | 은은한 초록빛, 잎과 꽃 장식, 편안한 여백과 자연스러운 사진 흐름 |

- 생성한 이미지는 컨셉을 보조하는 장식에 사용하며, 고객이 저장한 대표 사진이나 갤러리 사진을 대신하지 않습니다.
- 테마별 공유 영역도 본문과 같은 배경·글자·테두리 색으로 연결합니다.
- 디자인 이름과 표시 순서만 갱신하고 기존 테마 키·공개 주소·판매 정책·Firebase 데이터 조회 및 저장 구조는 유지합니다.
