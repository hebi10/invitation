...
운영 홈(`view=home`)은 기존 조회 데이터로 현황과 최근 이벤트·방명록을 보여줍니다. 이벤트 상세는 모달 대신 목록을 대체하는 독립 작업 화면이며 기존 `event` 주소와 목록 필터를 유지합니다. 상세 메뉴는 기본 정보·디자인·공개·노출·고객·이미지 등 지원 기능별로 나눕니다. PC 필터는 항상 노출하고, 고객은 연결 이벤트를 먼저 확인하며 방명록 삭제는 작업 메뉴에서 실행합니다. 닫기에는 생성한 투명 X 이미지(`/images/admin/close.webp`)와 접근 가능한 버튼 이름을 함께 사용합니다. 상세 탭 및 사이드바 이동은 공통 저장 확인을 거칩니다.
...
초대장 편집기는 관리자만 시작 설정에서 생성·디자인·주소·고객 연결을 관리한다. 고객에게는 기본 정보부터 표시하고 이름·일정·장소·사진·문구를 편집하게 한다. 등급 선택은 두지 않으며 기존 등급 데이터도 프리미엄 기능으로 표시한다. PC의 실제 청첩장 미리보기는 입력 옆 휴대폰 화면에, 모바일에서는 별도 미리보기 창에 표시한다. 미입력 예시는 저장 데이터와 분리한다.

관리자 생성·연결 화면은 `/page-wizard/edit`로 분리한다. 기존 `/page-wizard`는 생성 화면으로 안내하고, `/page-wizard/[slug]`는 관리자와 고객 모두 기본 정보부터 입력하는 화면이다. 이름은 상단 작업 제목과 분리한 보조 정보로 표시하고, 일정·장소 입력 사이에 여백을 둔다. 지도 확인 영역은 중립색 정보행과 각진 지도 영역을 사용한다.
