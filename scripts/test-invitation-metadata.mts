...
import {
  ...
  getPublicSiteUrl,
} from '../src/lib/invitationMetadata';
...
assert.equal(DEFAULT_PUBLIC_SITE_URL, 'https://invite.msgnote.kr');
assert.equal(getPublicSiteUrl('https://preview.msgnote.kr/').toString(), 'https://preview.msgnote.kr/');
assert.equal(getPublicSiteUrl('javascript:alert(1)').toString(), 'https://invite.msgnote.kr/');

assert.equal(metadataBase, 'https://invite.msgnote.kr/');
...
assert.equal(
  ...
  'https://invite.msgnote.kr/images/sample-wedding-romantic.webp'
);
