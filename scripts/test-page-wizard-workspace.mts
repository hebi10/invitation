...
assert.deepEqual(setupOnlySteps.map(step => step.key), ['slug'], '웨딩 생성 화면에는 디자인 선택 없이 주소 설정만 표시합니다.');
assert.equal(weddingSteps.some(step => step.key === 'theme'), false, '웨딩 초대장은 디자인 선택을 요구하지 않습니다.');
