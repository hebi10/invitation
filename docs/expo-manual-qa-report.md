...
| 항목 | 판정 | 근거 |
| --- | --- | --- |
| 고객 로그인 | 정상 | `create.tsx`/`login.tsx` -> `AuthContext.loginCustomer` -> `loginMobileCustomerAuth` -> `/api/mobile/auth/login` 경로가 유지되고, 모바일 typecheck/lint 통과 |
...
| ownerUid 기준 인증 연결 확인 | 정상 | 모바일 생성/구매 이행 시 Firebase UID를 이벤트 `ownerUid`에 연결하고, owner session으로 운영 API를 사용 |

## 2-6. QA 결과 정리
### 정상
- 모바일 타입체크와 린트는 통과했다.
- 고객 로그인, 최근 연동 목록, 주소 입력, 생성 요청 구조, 공개 상태 변경, 방명록 관리, 재인증 모달, 링크 재발급, 기존 데이터의 `events`/`eventSlugIndex` 경로는 코드 기준으로 정상이다.
