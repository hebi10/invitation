...
## 1. 한 줄 소개
모바일 청첩장의 생성, 공개, 고객 수정, 운영 관리, 추억 페이지 확장까지 한 흐름으로 다루는 Next.js + Firebase 기반 운영형 서비스입니다.

단순히 예쁜 청첩장 화면을 만드는 프로젝트가 아니라, 관리자가 여러 청첩장을 운영하고 고객이 자기 페이지를 직접 수정하며 하객이 공개 페이지를 보는 실제 서비스 흐름을 목표로 합니다.

## 2. 서비스가 해결하는 문제
- 청첩장 생성과 수정 과정이 운영자에게 반복 부담이 되는 문제
- 고객이 작은 문구나 이미지 수정 때마다 운영자에게 의존해야 하는 문제
...
| 고객 | 비밀번호로 편집기 진입, 문구/이미지/일정 수정 | 운영자 도움 없이 자기 청첩장 수정 |
| 관리자 | 청첩장 생성, 공개 상태, 노출 기간, 비밀번호, 이미지 관리 | 여러 페이지를 한 콘솔에서 운영 |
...
5. 고객은 `/page-editor/{slug}`에서 권한 확인 후 내용을 직접 수정합니다.
...
| `/page-editor` | 고객 편집 시작 | 고객용 draft 생성 또는 진입 | `screenshots/09-page-editor-create.png` |
| `/page-editor/{slug}` | 고객 편집기 | 권한 확인, 자동 저장, 미리보기 | `screenshots/10-page-editor-detail.png` |
...
| 권한 | 관리자, 고객 소유자, 공개 방문자를 API와 rules에서 분리 |
...
### 고객 편집 저장
고객은 권한 확인 후 API를 통해 저장하고, 서버는 repository 계층을 통해 Firestore에 기록합니다. 직접 Firestore 접근은 경계 테스트에서 차단합니다.
...
- 관리자와 고객의 권한을 분리한 운영 모델
- 공개 상태와 표시 기간을 함께 다루는 실제 운영 정책
- 테마 렌더러와 이벤트 타입을 확장할 수 있는 구조
- repository 경계와 QA 스크립트로 데이터 접근 원칙을 검증하는 흐름

## 9. 면접 설명 예시
> 모바일 청첩장을 단순히 보여주는 화면이 아니라, 관리자가 생성하고 고객이 직접 수정하며 하객이 확인하는 운영형 서비스로 만들었습니다. Firestore의 이벤트 도메인을 source of truth로 두고, 공개 URL은 slug index로 해석하며, 권한이 필요한 작업은 API와 repository 계층을 거치도록 정리했습니다.

기술 포인트를 설명할 때는 다음 순서가 좋습니다.
1. 왜 운영형 구조가 필요한지 설명합니다.
2. 관리자, 고객, 하객의 권한과 화면을 분리한 이유를 설명합니다.
