...
- 관리자 비밀번호 목록은 전용 API(`/api/admin/passwords`)를 사용합니다.
- 관리자 요약, 페이지 목록, 댓글 목록, 비밀번호 목록, 고객 계정 목록은 모두 React Query 기반이며 `staleTime 45초`, `gcTime 10분`, `refetchOnWindowFocus: false`를 사용합니다.

## 현재 동작

1. 상단 헤더
   - `요약 새로고침` 버튼으로 요약 API만 수동 재조회합니다.
2. 페이지/댓글/비밀번호/고객 계정 탭
