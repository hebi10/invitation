...
interface LocationMapProps {
  appearance?: 'simple' | 'natural' | 'classic';
  address: string;
  venueName: string;
  kakaoMapConfig?: {
    latitude: number;
...
export default function LocationMap({ address, venueName, kakaoMapConfig, mapHref, appearance }: LocationMapProps) {
  ...
  return (
    <div ref={sectionRef} className={styles.locationMap} data-appearance={appearance}>
      ...
      {(appearance === 'natural' || appearance === 'classic') && status === 'ready' ? <button className={styles.mapControl} type="button" onClick={toggleControls} aria-pressed={controlsEnabled}>
        {controlsEnabled ? '지도 조작 마치기' : '지도 확대·이동'}
      </button> : null}
      {status === 'ready' && (
        <div className={styles.actions}>
          {(appearance === 'natural' || appearance === 'classic') && directionsHref ? <a href={directionsHref} target="_blank" rel="noopener noreferrer">지도 앱에서 길찾기</a> : <button type="button" onClick={toggleControls} aria-pressed={controlsEnabled}>
