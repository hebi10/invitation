...
import { 
  Cover_5 as Cover, 
  Greeting_5 as Greeting, 
  Gallery_5 as Gallery, 
  Schedule_5 as Schedule, 
  LocationMap_5 as LocationMap, 
  WeddingCalendar_5 as WeddingCalendar,
  GiftInfo_5 as GiftInfo, 
  Guestbook_5 as Guestbook,
  WeddingLoader_5 as WeddingLoader
} from '@/components';
...
export default function ShinMinjeKimHyunjiClassic() {
  ...
  useEffect(() => {
    let canceled = false;
    document.title = `${pageConfig?.groomName || ''} ♡ ${pageConfig?.brideName || ''} 결혼식 - ${pageConfig?.date || ''} (한지 클래식 버전)`;
  ...
  return (
    <main role="main" aria-label={`${pageConfig?.groomName || ''}와 ${pageConfig?.brideName || ''}의 결혼식 청첩장 (한지 클래식 버전)`}>
      <Cover 
