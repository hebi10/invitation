...
export default function PageWizardClient({
  ...
  useEffect(() => {
    if (isAdminLoading || isLoading || wizardSteps.some((step) => step.key === activeStepKey)) {
      return;
    }

    setActiveStepKey(wizardSteps[0]?.key ?? 'basic');
  }, [activeStepKey, wizardSteps, isAdminLoading, isLoading]);
  ...
  const handleCreateInSetup = async () => {
    if (!isAdminLoggedIn || isSaving || resolvedPersistedSlug) return;
    const setup = wizardSections.find(section => section.id === 'setup');
    if (!setup) return;
    const validation = getWizardSectionValidation(setup, getValidationForStep);
    setInteractedStepKeys(current => new Set([...current, ...setup.steps.map(step => step.key)]));
    if (!validation.valid) {
      moveToStep(validation.invalidStepKeys[0]);
      showErrorNotice(validation.messages[0], undefined, 'validation');
      return;
    }
      ...
      <main className={pageClassName} data-operation-ui>
        <div className={`${styles.shell} ${styles.gateShell}`}>
          ...
          <section className={`${styles.centerCard} ${styles.gateCard}`}>
            ...
            <h1 className={styles.centerTitle}>관리자의 고객 연결이 필요합니다.</h1>
            <p className={styles.centerText}>관리자가 이 초대장을 고객 계정에 연결한 뒤 편집할 수 있습니다. 연결을 요청한 후 다시 확인해 주세요.</p>
