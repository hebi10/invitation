...
export default function Gallery_4({ images }: GalleryProps) {
  ...
  return (
    <section ref={elementRef as RefObject<HTMLElement>} className={styles.gallery} aria-label="갤러리">
      <div className={styles.container}>
        <div className={styles.header}>
          <div className={styles.lemonDecoration}>◌</div>
          <h2 className={styles.title}>Gallery</h2>
          <p className={styles.subtitle}>함께한 순간을 천천히 다시 봅니다</p>
        </div>
        ...
        <div className={styles.grid}>
          {(isVisible ? displayImages : Array.from({ length: Math.min(6, Math.max(images.length, 4)) }, (_, index) => `placeholder-${index}`)).map((image, index) =>
            image.startsWith('placeholder-') ? (
              <div key={image} className={styles.imageCard}>
        ...
        {images.length > 6 && (
          <div className={styles.loadMoreContainer}>
            {hasMore ? (
              <button className={styles.loadMoreButton} onClick={() => setVisibleCount((current) => Math.min(current + 6, images.length))} aria-label="이미지 더 보기" type="button">
                <span>더 보기</span>
                <span className={styles.loadMoreIcon}>↓</span>
              </button>
            ) : (
              <button className={styles.loadMoreButton} onClick={() => setVisibleCount(6)} aria-label="이미지 접기" type="button">
                <span>접기</span>
                <span className={styles.loadMoreIcon}>↑</span>
              </button>
            )}
