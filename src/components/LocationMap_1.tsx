...
export default function LocationMap_1({
  ...
  useEffect(() => {
    ...
    void loadKakaoMapsSdk()
      ...
      .catch((error) => {
        console.error('[LocationMap_1] failed to load Kakao Maps SDK', error);
  ...
  const initializeKakaoMap = () => {
    ...
    try {
      ...
      geocoder.addressSearch(address, (result: any, status: any) => {
        ...
        console.error('[LocationMap_1] address search failed', status);
        setKakaoMapLoaded(true);
      });
    } catch (error) {
      console.error('[LocationMap_1] failed to initialize map instance', error);
      setKakaoMapLoaded(true);
    }
  };
